...
def extract_code_from_response(response: str) -> str:
    """
    Extracts code from a Ollama response, handling various markdown code block formats.
    ...
    if match:
        ...
        print(f"{Fore.YELLOW}Warning: No code block found in Ollama response, returning full response")
        return response
...
def generate_code_with_ollama(prompt: str) -> str:
    """Generates code using Ollama's API."""
    global DEBUG_MODE
    if DEBUG_MODE:
         print(f"{Fore.MAGENTA}{Style.BRIGHT}---DEBUG: Prompt sent to Ollama---{Style.RESET_ALL}")
         print(f"{Fore.WHITE}{prompt}{Style.RESET_ALL}")

    try:
        response = ollama.generate(model=OLLAMA_MODEL, prompt=prompt)
        if DEBUG_MODE:
              print(f"{Fore.MAGENTA}{Style.BRIGHT}---DEBUG: Response received from Ollama---{Style.RESET_ALL}")
              print(f"{Fore.WHITE}{response['response']}{Style.RESET_ALL}")
        return response['response']
    except Exception as e:
        print(f"{Fore.RED}Error communicating with Ollama: {e}")
        return ""
...
{test_code}"""
                ...
                print(f"\n{Fore.MAGENTA}Attempt {attempts + 1}/{max_attempts}: Asking Ollama to generate code for test: {test_name}...")

                code = generate_code_with_ollama(prompt)

                if not code:
                     print(f"{Fore.RED}Error generating code for test {test_name}, skipping test.")
                     break
    ...
    def extract_test_code(self, test_content: str, test_name:str) -> str:
         test_code_regex = re.compile(
            r"it\s*\(\s*['\"]" + re.escape(test_name) + r"['\"]\s*,\s*\(.*\)\s*=>\s*\{([\s\S]*?)\}\s*\)", re.MULTILINE
            )
         match = test_code_regex.search(test_content)
         if match:
...
def main():
    global DEBUG_MODE
    parser = argparse.ArgumentParser(description="Run TDD with Ollama.")
    ...
    parser.add_argument(
        "--model", "-m", type=str, default="deepseek-r1", help="The ollama model to use"
    )
